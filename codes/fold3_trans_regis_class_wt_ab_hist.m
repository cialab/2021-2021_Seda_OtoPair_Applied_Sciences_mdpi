...
% load('\\medctr.ad.wfubmc.edu\dfs\cialab$\scratch\cialab\scamalan\Autoscope\Pair\normal_abnormal_ent_log_Errors.mat');
...
folder = 'wo_reg/' ;  %'wo_reg/' %'w_reg/'  %'reg_hist_std/' %'reg_hist_std-mean/'  %'reg_hist/'
     

n = [1;1]*(1:p);
idx = n(:)';
fld=3;
eighteen = ones(18,1);       seventeen = ones(17,1);      
twenty= ones(20,1);         twentyone= ones(21,1); 

th_six = ones(36,1);       th_four = ones(34,1);      
fourthy= ones(40,1);       fourthytwo= ones(42,1); 

fold = [reshape(th_six*(1:2),[1 72]),reshape(th_four*(3:3),[1 34]),...
    reshape(fourthy*(1:2),[1 80]),reshape(fourthytwo*(3:3),[1 42])];
half_fold = [reshape(eighteen*(1:2),[1 36]),reshape(seventeen*(3:3),[1 17]),...
    reshape(twenty*(1:2),[1 40]),reshape(twentyone*(3:3),[1 21])];
