...
folder = 'reg_hist_std-mean/' ;  %'wo_reg/' %'w_reg/'  %'reg_hist_std/' %'reg_hist_std-mean/'  %'reg_hist/'
     

n = [1;1]*(1:p);
idx = n(:)';
fld=3;
sixteen = ones(16,1);       %seventeen = ones(17,1);      
twentytwo= ones(22,1);         %twentyone= ones(21,1); 
th_two= ones(32,1);       %th_four = ones(34,1);      
fourthy_four= ones(44,1);       %fourthytwo= ones(42,1); 

fold = [reshape(th_two*(1:3),[1 96]),...%reshape(th_four*(3:3),[1 34]),...
    reshape(fourthy_four*(1:3),[1 132])];%,reshape(fourthytwo*(3:3),[1 42])];
half_fold = [reshape(sixteen*(1:3),[1 48]),...%reshape(seventeen*(3:3),[1 17]),...
    reshape(twentytwo*(1:3),[1 66])];%,reshape(twentyone*(3:3),[1 21])];
...
%     save(strcat('/isilon/datalake/cialab/scratch/cialab/scamalan/Autoscope/Pair/Normal_Abnormal/ENT_3fold_wo_sd_all_data/',folder,'net',string(i),'.mat'),'net');
...
% save(strcat('/isilon/datalake/cialab/scratch/cialab/scamalan/Autoscope/Pair/Normal_Abnormal/ENT_3fold_wo_sd_all_data/',folder,'acc_conf',string(i),'.mat'),'test_accuracy','test_C','val_accuracy','val_C','train_accuracy','train_C');
    ...
    w=net.Layers(822,1).Weights;
%     save(strcat('/isilon/datalake/cialab/scratch/cialab/scamalan/Autoscope/Pair/Normal_Abnormal/ENT_3fold_wo_sd_all_data/',folder,'weights',string(i),'.mat'),'w');
    set = featuresTrain;
    lookup_train=set*w';
...
%         add these values to data
        data(k,9:18) = I1a_hist;
        data(k,19:28) = I1b_hist;
        data(k,29:38) = I2a_hist;
        data(k,39:48) = I2b_hist;
%         
%         data(k,49:52) = [mean_1a mean_1b mean_2a mean_2b];
...
%         data(k,61:64) = [kurtosis_1a kurtosis_1b kurtosis_2a kurtosis_2b];
        ...
        data(k,49:52) = [std_1a std_1b std_2a std_2b];
        data(k,53:56)  = [skewness_1a skewness_1b skewness_2a skewness_2b];
        data(k,57:60) = [kurtosis_1a kurtosis_1b kurtosis_2a kurtosis_2b];
...
%     saveas(plt,strcat('/isilon/datalake/cialab/scratch/cialab/scamalan/Autoscope/Pair/Classification_results/ENT_normal_abnormal/3fold_wo_sd_all_data/',folder,'fold-',string(i),'_ROC.fig'));
%     saveas(plt,strcat('/isilon/datalake/cialab/scratch/cialab/scamalan/Autoscope/Pair/Classification_results/ENT_normal_abnormal/3fold_wo_sd_all_data/',folder,'fold-',string(i),'_ROC.png'));
...
%         pre_tru(i,1)=(test_label(1));
        i
close all
% save(strcat('/isilon/datalake/cialab/scratch/cialab/scamalan/Autoscope/Pair/Normal_Abnormal/ENT_3fold_wo_sd_all_data/',folder,'feat_data_fold',string(i),'.mat'),'data');
...
% save(strcat('/isilon/datalake/cialab/scratch/cialab/scamalan/Autoscope/Pair/Classification_results/ENT_normal_abnormal/3fold_wo_sd_all_data/',folder,'result.mat'),'acc','correct_list','pre_tru','conf','std_dev','all_scores','half_fold','gen_acc','gen_std_dev');
...
[XN,YN,T,AUC,OPTROCPT] = perfcurve(pre_tru(:,1),all_scores(:,2),1);
    ...
    ylabel('True positive rate')
    legend('Normal','Abnormal','Location','Best');
    title(strcat('ROC for ALL'));
% saveas(plt,strcat('/isilon/datalake/cialab/scratch/cialab/scamalan/Autoscope/Pair/Classification_results/ENT_normal_abnormal/3fold_wo_sd_all_data/',folder,'ALL_ROC.fig'));
%     saveas(plt,strcat('/isilon/datalake/cialab/scratch/cialab/scamalan/Autoscope/Pair/Classification_results/ENT_normal_abnormal/3fold_wo_sd_all_data/',folder,'ALL_ROC.png'));
